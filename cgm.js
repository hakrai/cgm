(function () {
    ...
    function defaultData() {
        return {
            ...
            settings: { scrollDelay: 2, sidebarPosition: 'right', accent: '#6366f1', sidebarOpen: true },
    ...
    function scanLinks(container) {
        ...
        links.forEach(a => {
            ...
            if (DATA.chats[id]) {
                ...
                DATA.chats[id] = { id, name: name || id, url, groupId: null, pinned: false, customName: false };
    ...
    async function handleRefresh(btn) {
        ...
        if (!container) {
            alert('⚠️ Original chat sidebar not found.\n\nPlease open the sidebar first, then click Refresh again.');
            return;
        }
    ...
    function closeAllPopups() {
        document.querySelectorAll('.cgm_popup, .cgm_popup_overlay').forEach(el => el.remove());
    }
    ...
    function buildChatEl(chat) {
        ...
        li.className = 'cgm_chat_item' + (chat.pinned ? ' cgm_pinned_item' : '');
      ...
      <button class="cgm_icon_btn cgm_more_btn" draggable="false" title="Actions">${icon('more-vertical')}</button>`;
        li.querySelector('.cgm_chat_name').addEventListener('click', () => { window.location.href = chat.url; });
    ...
    function buildGroupEl(group) {
      ...
      <div class="cgm_group_header" data-dnd="group" data-id="${group.id}" draggable="true">
        <span class="cgm_grip">${icon('grip-vertical')}</span>
        <button class="cgm_icon_btn" data-act="toggle" draggable="false">${icon(effectiveCollapsed ? 'chevron-right' : 'chevron-down')}</button>
        <span class="cgm_group_name">${escapeHtml(group.name)}</span>
        <span class="cgm_badge">${validIds.length}</span>
        <button class="cgm_icon_btn" data-act="rename" draggable="false" title="Rename">${icon('pencil')}</button>
        <button class="cgm_icon_btn" data-act="delete" draggable="false" title="Delete">${icon('trash-2')}</button>
      </div>
      <ul class="cgm_chat_list" data-container-type="group" data-container-id="${group.id}"></ul>`;
        ...
        div.querySelector('[data-act="toggle"]').addEventListener('click', () => { group.collapsed = !group.collapsed; saveData(); renderAll(); });
        div.querySelector('[data-act="rename"]').addEventListener('click', (e) => {
            openPopup({ title: 'Rename group', value: group.name, anchorEl: e.currentTarget, onSave: (v) => { group.name = v; saveData(); renderAll(); } });
        });
        div.querySelector('[data-act="delete"]').addEventListener('click', () => {
            if (confirm(`Delete group "${group.name}"? Its chats will move to Ungrouped.`)) deleteGroup(group.id);
        });
        return div;
    }
    ...
    function toggleSettingsPanel() {
        const sp = panelEl.querySelector('#cgm_settings_panel');
        const scrollArea = panelEl.querySelector('#cgm_scroll_area');
        const isOpen = sp.style.display !== 'none';
        sp.style.display = isOpen ? 'none' : 'block';
        scrollArea.style.display = isOpen ? 'flex' : 'none';
    }
    function exportData() {
    ...
    function buildSettingsPanel() {
        ...
        sp.innerHTML = `
      <div class="cgm_settings_header">
        <button class="cgm_icon_btn" id="cgm_settings_back" draggable="false">${icon('chevron-left')}</button>
        <b>Settings</b>
      </div>
      <div class="cgm_settings_body">
        ...
        sp.querySelector('#cgm_settings_back').onclick = toggleSettingsPanel;
        const slider = sp.querySelector('#cgm_delay_slider');
        slider.addEventListener('input', () => {
            const v = parseFloat(slider.value);
            DATA.settings.scrollDelay = v;
            sp.querySelector('#cgm_delay_label').textContent = v + 's';
            sp.querySelector('#cgm_wait_label').textContent = Math.max(v - 0.5, 0.3).toFixed(1) + 's';
            saveData();
        });
        sp.querySelectorAll('.cgm_seg_btn').forEach(btn => {
            btn.onclick = () => {
                DATA.settings.sidebarPosition = btn.dataset.pos;
                saveData(); applySettingsToDOM();
                sp.querySelectorAll('.cgm_seg_btn').forEach(b => b.classList.toggle('active', b === btn));
            };
        });
        sp.querySelectorAll('.cgm_swatch').forEach(btn => {
            btn.onclick = () => {
                DATA.settings.accent = btn.dataset.color;
                saveData(); applySettingsToDOM();
                sp.querySelectorAll('.cgm_swatch').forEach(b => b.classList.toggle('active', b === btn));
                sp.querySelector('#cgm_custom_color').value = btn.dataset.color;
            };
        });
        sp.querySelector('#cgm_custom_color').addEventListener('input', (e) => { DATA.settings.accent = e.target.value; saveData(); applySettingsToDOM(); });
        sp.querySelector('#cgm_export_btn').onclick = exportData;
        sp.querySelector('#cgm_import_btn').onclick = () => sp.querySelector('#cgm_import_file').click();
        sp.querySelector('#cgm_import_file').addEventListener('change', (e) => {
            ...
            reader.onload = () => {
                try {
                    ...
                    saveData(); renderAll(); buildSettingsPanel();
                    alert('✅ Backup imported successfully.');
                } catch (err) { alert('❌ Failed to import backup: ' + err.message); }
            };
            reader.readAsText(file);
        });
        sp.querySelector('#cgm_reset_btn').onclick = () => {
            if (confirm('This will erase all groups, pins and renames. Continue?')) {
                DATA = defaultData(); saveData(); renderAll(); buildSettingsPanel();
            }
        };
    }
      ...
      <div id="cgm_panel" class="cgm_panel">
        <div class="cgm_header">
          ...
          <div class="cgm_search_wrap">
            ...
            <input type="text" id="cgm_search_input" placeholder="Search chats...">
          </div>
        ...
        <div class="cgm_scroll_area" id="cgm_scroll_area">
          <div class="cgm_section" id="cgm_pinned_section">
            <div class="cgm_section_header">
              <button class="cgm_icon_btn" data-act="toggle-pinned" draggable="false"></button>
          ...
          <div class="cgm_section" id="cgm_ungrouped_section">
            <div class="cgm_section_header">
              <button class="cgm_icon_btn" data-act="toggle-ungrouped" draggable="false"></button>
        ...
        <div class="cgm_settings_panel" id="cgm_settings_panel" style="display:none;"></div>
      </div>`;
    ...
    function bindStaticControls() {
        ...
        panelEl.querySelector('#cgm_settings_btn').addEventListener('click', toggleSettingsPanel);
        ...
        panelEl.querySelector('[data-act="toggle-pinned"]').addEventListener('click', () => {
            DATA.collapse.pinned = !DATA.collapse.pinned; saveData(); renderAll();
        });
        panelEl.querySelector('[data-act="toggle-ungrouped"]').addEventListener('click', () => {
            DATA.collapse.ungrouped = !DATA.collapse.ungrouped; saveData(); renderAll();
        });

        // Auto-close sidebar when clicking outside (but ignore clicks on popups/menus which live outside #cgm_root)
        document.addEventListener('mousedown', (e) => {
            if (!DATA.settings.sidebarOpen) return;
            if (rootEl.contains(e.target)) return;
            if (e.target.closest && (e.target.closest('.cgm_popup') || e.target.closest('.cgm_popup_overlay'))) return;
    ...
    function deleteChat(chatId) {
        ...
        if (!confirm(`Delete "${label}" from Chat Groups?\n\nThis only removes it from the manager (pins/groups/name), it does NOT delete the actual chat from the website.`)) return;
    ...
    .cgm_toggle_tab{ display:none; position:absolute; top:50%; transform:translateY(-50%); width:22px; height:56px; background:var(--cgm-accent); color:#fff; align-items:center; justify-content:center; cursor:pointer; box-shadow:0 2px 10px rgba(0,0,0,.35); }
    ...
    .cgm_section_header{ display:flex; align-items:center; gap:6px; margin-bottom:6px; cursor:pointer; user-select:none; }
    ...
    .cgm_chat_name{ flex:1; font-size:13px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; }
    .cgm_mark{ background:var(--cgm-accent); color:#fff; border-radius:3px; padding:0 2px; }
    ...
    .cgm_btn_primary{ background:var(--cgm-accent); border-color:var(--cgm-accent); color:#fff; }
    ...
    .cgm_group_header{ display:flex; align-items:center; gap:6px; padding:8px; cursor:grab; }
    ...
    .cgm_group .cgm_chat_list{ padding:0 6px 6px 6px; }
    .cgm_group.cgm_collapsed .cgm_chat_list{ display:none; }
    .cgm_popup{
      position:fixed;
      z-index:2147483647;
      background: var(--cgm-bg-elevated, #20212c) !important;
      opacity:1 !important;
      border:1px solid var(--cgm-border, #31323f);
      border-radius:10px;
      padding:10px;
      width:230px;
      box-shadow:0 10px 32px rgba(0,0,0,.55), 0 0 0 1px rgba(255,255,255,.04);
      color: var(--cgm-fg, #e7e7ee);
    }
    ...
    .cgm_popup_input{ width:100%; padding:6px 8px; background: var(--cgm-bg, #181920) !important; border:1px solid var(--cgm-border); border-radius:6px; color:var(--cgm-fg); font-size:13px; margin-bottom:8px; }
    .cgm_popup_actions{ display:flex; justify-content:flex-end; gap:6px; }
    .cgm_menu{ padding:6px; display:flex; flex-direction:column; background: var(--cgm-bg-elevated, #20212c) !important; }
    ...
    .cgm_settings_panel{ flex:1; overflow-y:auto; padding:12px; }
    .cgm_settings_header{ display:flex; align-items:center; gap:8px; margin-bottom:12px; }
    ...
    .cgm_seg_btn{ flex:1; padding:7px; background:var(--cgm-bg-elevated); border:none; color:var(--cgm-fg); cursor:pointer; font-size:13px; }
    .cgm_seg_btn.active{ background:var(--cgm-accent); color:#fff; }
    ...
    #cgm_custom_color{ width:28px; height:28px; padding:0; border:none; background:none; cursor:pointer; }
